...
<body>
	...
	<script>
		...
		document.querySelector("#myForm2").addEventListener("submit" , (e)=>{
			e.preventDefault();
			// gura_util.js 안에 있는 ajaxFormPromise() 함수 활용
		...
		/*
			폼 안에 있는 submit 버튼을 누르면 폼 요소에는 "submit" 이벤트 발생
			따라서 "submit" 이벤트가 발생 시 원하는 동작이 있다면
			.addEventListener("submit" , ()=>{}) 을 활용
		*/
		document.querySelector("#myForm").addEventListener("submit" , (e)=>{
			// event 객체 안에 있는 preventdefault() 함수를 호출하면 폼 제출 막기
			e.preventDefault();
			// fetch 함수를 이용해 페이지 전환없이 입력한 내용을 읽어와서 직접 제출하기
			
			// 입력한 내용을 query 문자열로 읽어오기
			const data = new FormData(e.target);
			const queryString = new URLSearchParams(data).toString();
			
			// fetch() 함수를 post 로 전송하기
			fetch("login.jsp" , {
				method:"POST",
